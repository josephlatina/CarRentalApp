...
export default function CollapsibleTable() {
    ...
    const [branches, setBranches] = us(eSt)ate([]);
    ...
    function searchFor(toSearch) {
        ...
        for (var i = 0; i < rentals.length; i++) {
            for (var key in rentals[i]) {
                if (
                    rentals[i][key]?.toString().toLowerCase().indexOf(toSearch) !== -1 ||
                    ""
                ) {
                    if (!itemExists(results, rentals[i]))
                        results.push(rentals[i]);
                }
            }
        }
        return results;
